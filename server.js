...
function serveStatic(reqPath, res) {
  ...
  try {
    filePath = safeJoin(publicDir, relativePath);
  } catch (error) {
...
function computeAssessmentMetrics(record) {
  const asarAnswers = (record.tests?.asar || [])
    .map((item) => Number(item.answer))
    .filter((value) => Number.isFinite(value));
  const dsmAnswers = (record.tests?.dsm5 || [])
    .map((item) => Boolean(item.answer));

  const asarAverage = asarAnswers.length
    ? asarAnswers.reduce((sum, value) => sum + value, 0) / asarAnswers.length
    : 0;
  const dsmYesCount = dsmAnswers.filter(Boolean).length;

  const attention = clamp(100 - asarAverage * 14, 20, 95);
  const executive = clamp(100 - asarAverage * 16, 18, 92);
  const impulse = clamp(100 - dsmYesCount * 10, 20, 90);
  const emotion = clamp(82 - dsmYesCount * 5 - asarAverage * 4, 18, 88);
  const structure = clamp(85 - asarAverage * 12, 20, 90);

  const severity = asarAverage >= 4 || dsmYesCount >= 5
    ? "높음"
    : asarAverage >= 3 || dsmYesCount >= 3
      ? "중간"
      : "낮음";

  return {
    asarAverage: Number(asarAverage.toFixed(2)),
    dsmYesCount,
    severity,
    scores: {
      attention,
...
function buildInsightsPrompt(record, metrics) {
  return [
    ...
    "2. report.sections.summary는 2문장 이내",
    "3. report.sections.strength는 2문장 이내",
    "4. report.sections.watchout는 2문장 이내",
    "5. plan.suggestions는 한국어 문장 3개, 각각 구체적이고 실행 가능하게 작성",
    "6. plan.openingMessage는 사용자가 계획 수정을 요청할 수 있게 유도하는 1~2문장"
  ].join("\n");
}
...
function buildChatPrompt(record, message) {
  return [
    "서비스: soul.ai.kr ADHD 선별/감별 보조 모바일 웹 앱",
    "역할: 사용자의 기존 계획을 현실적으로 조정하는 한국어 AI 코치",
...
async function generateInsights(record) {
  ...
  const payload = await callGeminiJson({
    systemInstruction: "You are a clinical-style but non-diagnostic ADHD screening assistant for a Korean mobile web app. Return concise Korean JSON only.",
  ...
  return {
    report: {
      ...
      sections: {
        summary: payload.report?.sections?.summary || "응답을 기반으로 주의 조절의 기복이 관찰됩니다.",
        strength: payload.report?.sections?.strength || "흥미 기반 몰입은 보호 요인으로 보입니다.",
        watchout: payload.report?.sections?.watchout || "일상 구조화와 시작 지연 관리가 필요합니다."
      }
    },
    plan: {
